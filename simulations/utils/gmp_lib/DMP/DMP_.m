...
%             Fd_mse = mean((this.W*Phi - Fd).^2, 2)
        ...
        function [Time, y_data, dy_data, ddy_data] = generate_trajectory_2(this, y0, g,g_i, Tf, dt)
            ...
            for j = 1:num_timesteps
               ...
               % interpolation_factor = min(((j - 1)*1.5) / (num_timesteps - 1), 1);
                
               % interpolation_factor = (1.075*(j - 1) )/ (num_timesteps - 1);

                interpolation_factor = (1.115*j)/ (num_timesteps);
                interpolation_factor_2 = (1*j)/ (num_timesteps);
                original_distance = [0; 0; 0];
                desired_distance = [0.02;  -0.02; 0.02];
                interpolated_goal_pose_2 = (1 - interpolation_factor_2) * original_distance + interpolation_factor_2 * desired_distance;
                
                interpolated_goal = (1 - interpolation_factor) * g_i + interpolation_factor * g;
              %  fprintf('Interpolation Factor at step %d: %f\n', j, interpolation_factor);


                % Set interpolated goal
             %  this.setGoal(interpolated_goal);
               ...
               % disp('Coupling values at step:'), disp(j);
               % disp('Cdot:'), disp(result_Cdot);
               % disp('Combined Result:'), disp(combined_result);


                % Call coupling function
                % [C, Cdot] = this.coupling_2(y, interpolated_goal);
               ...
               % ddy = this.goal_attractor(y, dy, tau) + this.shape_attractor(s, s_dot, s_ddot, tau)  + this.coupling_force(combined_result, tau);
    ...
    methods (Abstract, Access = public)
        ...
        f_d = coupling_force(this, combined_result,tau)
