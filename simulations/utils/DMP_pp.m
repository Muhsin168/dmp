...
%             pause
        ...
        function f_d = coupling_force(this,combined_result,Tf )
        ...
        function [Time, y_data, dy_data, ddy_data] = generate_trajectory_2(this, y0, g,g_i, Tf, dt)
            ...
            for j=1:num_timesteps
                ...
                %interpolation_factor = (1.075*(j - 1) )/ (num_timesteps - 1);
                interpolation_factor = (1*j)/ (num_timesteps);
                interpolated_goal = (1 - interpolation_factor) * g_i + interpolation_factor * g;
             %   this.init(0, y0, interpolated_goal, Tf);

                original_distance = [0; 0; 0];
                desired_distance = [0.01;  -0.01; 0.01];
                %desired_distance = [0.00;  -0.5; 0.00];
                interpolated_goal_pose_2 = (1 - interpolation_factor) * original_distance + interpolation_factor * desired_distance;


                [~, result_Cdot] = this.coupling_2([], interpolated_goal_pose_2);

                % Combine results
                combined_result = zeros(3, 1);
                combined_result(1:3) = result_Cdot(4:6);
                
                % Print coupling values
                %disp('Coupling values at pp:'), disp(j);
                %disp('Cdot:'), disp(result_Cdot);
                %disp('Combined Result:'), disp(combined_result);
               ...
               ddy_data(:, j) = this.goal_attractor(y, y_dot, Tf) + this.shape_attractor(s, s_dot, s_ddot, Tf);
               % disp('Coupling values 1:'), ddy_data;
              
              
               % ddy_data(:, j) = this.goal_attractor(y, y_dot, Tf) + this.shape_attractor(s, s_dot, s_ddot, Tf)  + this.coupling_force(combined_result, Tf) ;
              %  ddy_data(:, j) = this.getRefAccel(s, s_dot, s_ddot);
               % fprintf('ddy_data(:, j):\n');
               % fprintf('%f\n', ddy_data(:, j));
                
               % ddy_data_2(:, j) = this.getRefAccel(s, s_dot, s_ddot);
               % fprintf('ddy_data_2(:, j):\n');
                %fprintf('%f\n', ddy_data_2(:, j));
            end
...
%                 dist = norm(pos - this.gmp0.getYd(s));
        ...
        function [C, Cdot] = coupling_2(this,y, y2)
            da = [0.0; 0.0; 0.0];
            
            desired_distance = y2; % y x z
            lf = [0.0; 1.0];
            k = 1.0;
            c1 = 0.1;
            c2 = 1000.0;
