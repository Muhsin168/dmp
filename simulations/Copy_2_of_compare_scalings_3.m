...
gap = 0.04; % Gap between subplots
margin = 0.08; % Margin around the figure
...
% ];
% 
% % Labels for DMP methods (exclude DMP++ for improved data)
% labels = {'DMP++', 'DMP classic', 'DMP bio plus', 'DMP bio', 'DMP rot'};
% 
...
% Data
original_data = [
    21.561017, 19.554797, 23.225967, 20.849018, 24.107387;  % Trial 1 (new data)
    11.762319, 16.109417, 16.004057, 13.654860, 15.432952;  % Trial 2 (new data)
    41.157986, 32.305616, 35.419813, 32.380733, 36.013383;  % Trial 3 (new data)
    13.714779, 41.991943, 34.800409, 39.467300, 36.784596;  % Trial 4 (new data)
    17.614975, 28.807353, 24.851279, 20.985578, 24.904620   % Trial 5 (new data)
];

improved_data = [
    13.538816, 14.370606, 14.424352, 15.054415;  % Trial 1 (new data)
    7.641860, 7.588698, 7.366076, 7.359234;      % Trial 2 (new data)
    24.118298, 22.111293, 23.224772, 23.250275;  % Trial 3 (new data)
    24.983661, 22.666521, 23.550735, 23.196005;  % Trial 4 (new data)
    15.705449, 13.485874, 12.368204, 13.464692   % Trial 5 (new data)
];

% Labels for DMP methods
labels = {'DMP-PP', 'DMP-C', 'DMP-BIE', 'DMP-BI', 'DMP-SS'};
...
groups = [repmat({'no DMP-GA'}, size(original_data, 1), 1); 
          repmat({' DMP-GA'}, size(improved_data, 1), 1)];
...
num_plots = size(data, 2); % Number of DMP methods
gap = 0.02; % Gap between subplots
margin = 0.1; % Margin around the figure
width = (1 - 2 * margin - (num_plots - 1) * gap) / num_plots; % Width of each subplot
...
% Adjust layout
% sgtitle('Comparison of Original and Improved Data for Each DMP Method (Excluding Improved DMP++)');

addpath('/home/muhsin/workspaces/plots/altmany-export_fig-3.46.0.0'); % Add the path to export_fig

% Save plot as PDF
% export_fig('2d_damp.pdf', '-pdf', '-q100', '-nocrop', '-transparent');


export_fig('CG_box.pdf', '-pdf', '-q100','-transparent');
